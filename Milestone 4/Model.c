...
RT_MUTEX autoStart;
RT_MUTEX txMutex[4]; //transmission mutex on buffer(s) send/recieve
enum { E_GAME, E_ADD, E_MOVE , E_WAIT }; //enums for MUTEX indices
...
unsigned int ballThreadSpeed = BTHREAD_RATIO;
...
char AImoveGenerate()
{
   ...
   n= 25*rand()/(RAND_MAX+1.0);
   temp =  pongs[opponent].leftCorner.point_x +n- ball.centre.point_x;
...
{
   fprintf(stderr, "Usage: %s -i <view ip> -b <ball speed>  -k <second view ip> \n",pname);
...
int main(int argc, char **argv)
{
    ...
    unsigned int port = MODEL_DEFAULT_PORT;
    char *ipAddress[2];
    ...
    int c;
    while((c = getopt(argc, argv, "i:b:k:")) != -1){
 	switch(c){
	   case 'i':
	     ipAddress[0] = strdup(optarg);
	     viewMode++;
	   break;

	  case 'k':
	     ipAddress[1] = strdup(optarg);
	     viewMode++;
	   break;
    ...
    view_addr[0].sin_addr.s_addr = inet_addr(ipAddress[0]);	
    view_addr[0].sin_port = htons(VIEW_DEFAULT_PORT);
    ...
    if(viewMode > 1){
 	   ...
 	   view_addr[1].sin_addr.s_addr = inet_addr(ipAddress[1]);	
 	   view_addr[1].sin_port = htons(VIEW_DEFAULT_PORT);
